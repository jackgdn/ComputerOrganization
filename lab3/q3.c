...
#define filterSize 3 // filter 3*3
#define matrixSize 3 // input 3*3 9elements
...
int main(int argc, char *argv[])
{
            ...
            for (j = 0; j < matrixSize; j++)
            {
                ...
                for (k = 0; k < filterSize; k++)
                {
                    for (l = 0; l < filterSize; l++)
                    {
                        ...
                        if (x >= 0 && y >= 0)
                            mat.n_val[i][j] += filter[k][l] * mat.o_val[x][y];
                    }
                }
            }
        }
    ...
    for (int i = 0; i < matrixSize; i++)
    {
        for (int j = 0; j < matrixSize - 1; j++)
            printf("%d ", mat.n_val[i][j]);

        printf("%d\n", mat.n_val[i][matrixSize - 1]);
    }
